  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((m) => [
        ...
        {
          role: 'assistant',
          content: 'Failed to reach Nexora AI. Please make sure the backend is running.',
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-4 border-b border-nexora-border bg-gradient-to-r from-white via-white to-nexora-accent/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-nexora-dark tracking-tight">
                    Nexora-Helper
                  </p>
                  <p className="text-[10px] text-nexora-dark/40 truncate max-w-[200px]">
                    {filename ?? datasetId.slice(0, 8)}
                  </p>
                </div>
              </div>
              <div className="flex gap-1">
                <button
                  ...
                  className="p-1.5 text-nexora-dark/40 hover:text-nexora-accent hover:bg-nexora-accent/5 rounded transition-colors"
                >
                  <Minimize2 className="w-3.8 h-3.8" />
                </button>
                <button
                  type="button"
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 min-h-[220px] bg-mesh-gradient">
              ...
              {messages.map((m, i) => (
                ...
                >
                  <div
                    className={`w-7.5 h-7.5 rounded-lg flex items-center justify-center shrink-0 border ${m.role === 'user' ? 'bg-nexora-dark/5 border-nexora-border' : 'bg-nexora-accent/10 border-nexora-accent/30'}`}
                  >
                    {m.role === 'user' ? (
                      <User className="w-3.8 h-3.8 text-nexora-dark/60" />
                    ) : (
                      <Bot className="w-3.8 h-3.8 text-nexora-accent" />
                    )}
                  </div>
                  <div
                    className={`text-sm px-3.5 py-2.5 rounded-2xl max-w-[82%] leading-relaxed whitespace-pre-wrap shadow-sm border ${m.role === 'user' ? 'bg-nexora-dark/5 border-nexora-border/50 text-nexora-dark' : 'bg-nexora-accent/5 border-nexora-accent/20 text-nexora-dark'}`}
                  >
                    {m.source && (
                      <span className="block text-[9px] font-bold uppercase tracking-widest text-nexora-accent mb-1.5 opacity-70">
                        {m.source}
                      </span>
                    )}
                    {m.content}
                ...
                <div className="flex gap-3 items-center animate-fade-in-up">
                  ...
                  <div className="text-sm px-3.5 py-2.5 rounded-2xl bg-gradient-to-r from-nexora-accent/10 to-nexora-accent/5 border border-nexora-accent/20 text-nexora-dark/60 font-medium inline-flex items-center gap-2">
                    <span className="inline-flex gap-1">
                      ...
                      <span
                        className="w-1.5 h-1.5 rounded-full bg-nexora-accent animate-pulse"
      ...
      >
        ...
        {open && minimized && (
          <span className="absolute -top-1 -right-1 w-3 h-3 bg-nexora-light rounded-full animate-pulse" />
        )}
      </motion.button>
    </>
  );
