      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 16 }}>
          ...
          <Bar dataKey={dataKey} radius={[0, 4, 4, 0]} opacity={0.9} isAnimationActive={false}>
            {chartData.map((_, i) => (
              <Cell key={i} fill={data.length ? (i % 2 === 0 ? "#fbbc05" : "#ea4335") : (i % 2 === 0 ? "#34a853" : "#4285f4")} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
      {!data.length && <p className="text-xs text-gray-400 mt-2">No missing values detected in the visible columns.</p>}
    </motion.div>
  );
}
