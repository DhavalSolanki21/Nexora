...
export default function ExperimentPanel({ datasetId }: { datasetId: string }) {
  ...
  const [comparison, setComparison] = useState<{ metric_names: string[]; rows: Record<string, unknown>[] } | null>(null);
    ...
    <motion.section className="glass p-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Training runs, production model versions, clustering, and forecasts.</p>
        </div>
      </div>

      {experiments.length === 0 ? (
        <p className="text-sm text-gray-400">No experiments recorded yet. Train models or run an exploration mode.</p>
      ) : (
            ...
            <thead>
              <tr className="text-left text-gray-400 border-b border-gray-100">
                ...
                {comparison?.metric_names.slice(0, 4).map((metric) => (
                  <th key={metric} className="py-3 pr-4">{metric}</th>
                ))}
            ...
            <tbody>
              {comparison?.rows.map((row) => (
                <tr key={String(row.run_id)} className="border-b border-gray-50">
                  <td className="py-3 pr-4 font-mono text-xs text-gray-400">{String(row.run_id).slice(0, 8)}</td>
                  ...
                  <td className="py-3 pr-4 text-xs text-gray-400">
                    {configSummary(experiments.find((exp) => exp.run_id === row.run_id)?.config ?? {})}
                  </td>
                </tr>
              ))}
            </tbody>
...
function formatMetric(value: unknown) {
  if (typeof value === "number") return Math.abs(value) <= 1 ? value.toFixed(4) : value.toLocaleString();
